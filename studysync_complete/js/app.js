...
function initUI() {
  const name = userDoc.name || currentUser.displayName || "Student";
...
async function uploadResourceFile(file, title, subject, desc) {
  ...
  if (file.size > 25*1024*1024) return toast("Max file size: 25MB", "error");
  uploadingFile = true;
  ...
  try {
    const path = `groups/${userDoc.groupId}/resources/${Date.now()}_${file.name}`;
    const sRef = storageRef(storage, path);
    const uploadTask = uploadBytesResumable(sRef, file);
    uploadTask.on('state_changed',
      (snap) => { const pct = Math.round((snap.bytesTransferred/snap.totalBytes)*100); if(progFill) progFill.style.width=pct+'%'; if(progTxt) progTxt.textContent=`Uploading... ${pct}%`; },
      (err) => { toast("Upload failed: "+err.message,"error"); uploadingFile=false; if(prog) prog.style.display='none'; },
      async () => {
        ...
        await addDoc(collection(db,"groups",userDoc.groupId,"resources"), { title,url,type:ft,subject,desc, fileName:file.name, fileSize:file.size, storagePath:path, addedBy:userDoc.name, addedByAvatar:userDoc.name.charAt(0).toUpperCase(), uid:currentUser.uid, createdAt:serverTimestamp() });
  ...
  el.innerHTML = filtered.map(r => {
    ...
    return `<div class="resource-card">
      <div class="res-card-header">
        ...
        ${isOwn?`<button class="res-delete-btn" onclick="deleteResource('${r.id}','${r.storagePath||''}')" title="Delete"><i class="fa-solid fa-trash"></i></button>`:''}
      </div>
      ...
      <div class="res-footer">
        ...
        ${r.url?`<a class="res-link" href="${r.url}" target="_blank"${r.fileName?` download="${r.fileName}"`:''}><i class="fa-solid fa-arrow-up-right-from-square"></i> ${r.fileName?'Download':'Open'}</a>`:''}
      </div>
    </div>`;
  }).join('');
}
